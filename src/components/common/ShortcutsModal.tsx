...
export const ShortcutsModal: React.FC = () => {
  ...
  const keyboardShortcuts = [
    { key: 'Ctrl + K', desc: 'Ouvrir la palette de commandes' },
    { key: 'Ctrl + S', desc: 'Sauvegarder immédiatement les données localement' },
    { key: 'Ctrl + F', desc: 'Placer le curseur dans la recherche globale' },
    { key: 'Ctrl + N', desc: 'Créer un nouvel enregistrement selon l\'onglet' },
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Contrôles ultra-rapides conçus pour la productivité métiers</p>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 gap-2">
              {keyboardShortcuts.map((sc, i) => (
                <div key={i} className="flex items-center justify-between p-2.5 rounded-lg bg-white/5 border border-white/5 hover:border-white/10">
                  ...
                  <kbd className="px-2 py-1 bg-black/40 border border-white/20 rounded text-xs font-mono text-cyan-300 font-semibold shadow-inner">
                    {sc.key}
                  </kbd>
                </div>
              ))}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 gap-2">
              {mouseShortcuts.map((sc, i) => (
                <div key={i} className="flex items-center justify-between p-2.5 rounded-lg bg-white/5 border border-white/5 hover:border-white/10">
                  <span className="text-sm text-slate-300">{sc.desc}</span>
                  <kbd className="px-2 py-1 bg-black/40 border border-white/20 rounded text-xs font-mono text-emerald-300 font-semibold shadow-inner">
                    {sc.key}
                  </kbd>
                </div>
              ))}
